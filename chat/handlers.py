        ...
        if session_id:
            ...
            if user_owner:
                ...
                yield from fanout_message(online_opponents_sockets,
                                          {'type': 'gone-online', 'usernames': [user_owner.username], 'chat_with': opponent_username})
            else:
    ...
    while True:
        ...
        if session_id and msg and username_opponent:
            ...
            if user_owner:
                ...
                Q(owner=user_owner, opponent=user_opponent) | Q(opponent=user_owner, owner=user_opponent))                
                print(dialog_status,"222222222222")
                dialogStatusOwner = dialog_status.dialog_status_owner
                dialogStatusOpponent = dialog_status.dialog_status_opponent
                if len(dialog) > 0: 
                    if dialogStatusOwner==True and dialogStatusOpponent == True:
...
@asyncio.coroutine
def seen_unseen_handler(stream):
    """
    Show message to opponent if user is typing message
    """
    while True:
        ...
        session_id = packet.get('session_key')
        user_opponent = packet.get('username')
        seen = packet.get('seen')
        message_id = packet.get('message_id')
        if session_id and user_opponent and seen and message_id is not None:
            user_owner = get_user_from_session(session_id)
            if user_owner:
                ...
                opponent_socket = ws_connections.get((user_opponent, user_owner.username))
                if seen and opponent_socket:
                    yield from target_message(opponent_socket,
                                              {'type': 'seen_unseen','username': user_owner.username,'message_id': message_id})
            else:
                pass  # invalid session id
        else:
            pass  # no session id or user_opponent or typing
...
def read_message_handler(stream):
    """
    Sendk message to user if the opponent has read the message
    """
    while True:
        packet = yield from stream.get()
        session_id = packet.get('session_key')
                    ...
                    if opponent_socket:
                        yield from target_message(opponent_socket,
                                                  {'type': 'opponent-read-message',
                                                   'username': user_owner, 'message_id': message_id})
                else:
